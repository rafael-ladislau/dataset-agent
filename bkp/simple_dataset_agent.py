...
def extract_list_from_response(response_text: str) -> List[str]:
    ...
    for marker in citation_markers:
        response_text = re.sub(rf'(?i){re.escape(marker)}.*?\n', '\n', response_text)
    ...
    try:
        ...
        logger.warning(f"Error extracting list from response: {str(e)}")
...
def filter_aliases(aliases: List[str], organizations: List[str] = None, remove_org_names: bool = False) -> List[str]:
    """
    ...
    - Optionally remove references to organizations and version numbers/years
    
    Args:
        aliases: List of aliases to filter
        organizations: List of organization names to remove from aliases
        remove_org_names: Whether to remove organization names from aliases (default: False)
    ...
    # Add lowercase versions of organizations for case-insensitive matching
    org_variants = set()
    if remove_org_names:
        for org in organizations:
            org_variants.add(org.lower())
            # Also add without punctuation
            org_variants.add(re.sub(r'[^\w\s]', '', org.lower()))
    
    # Process each alias
    processed_aliases = []
    
    # First pass: remove organization names and year/version numbers
    for alias in aliases:
        ...
        # Otherwise, process it by removing organization names if enabled
        ...
        # Remove organization names if requested
        if remove_org_names:
            for org in org_variants:
                # Only remove if it's a separate word or at beginning/end
                pattern = r'(?i)(?:^|\W)(' + re.escape(org) + r')(?:\W|$)'
    ...
    processed_aliases.sort(key=len)
    
    # Set for faster duplicate checking
    added = set()
    
    # Second pass: remove longer versions that contain shorter ones
    for alias in processed_aliases:
        alias_lower = alias.lower()
        
        # Skip if already added
        if alias_lower in added:
            continue
            
        # Check against already added aliases
        is_redundant = False
        
        # General substring relationship checking without hardcoded patterns
        # Check if this alias is redundant with any already added alias
        for added_alias in result:
            added_lower = added_alias.lower()
            
            # If added alias contains this one, this one is more specific (shorter) so keep it
            if added_lower in alias_lower:
                # This alias contains an already added one, but we'll still keep it
                # unless it's just adding articles or prepositions
                extra_words = set(alias_lower.split()) - set(added_lower.split())
                if extra_words and all(w in ('a', 'an', 'the', 'of', 'and', 'in', 'for') for w in extra_words):
                    is_redundant = True
                    break
            
            # If this alias is contained in an added one, it's likely a substring, so redundant
            if alias_lower in added_lower:
                is_redundant = True
                break
        
        if not is_redundant:
            result.append(alias)
            added.add(alias_lower)
...
"""
    ...
    try:
        ...
        # Generate common variations for specific dataset types
        additional_aliases = generate_more_alias_variations(aliases, dataset_name)
        
        # Add additional common variations
        aliases.extend(additional_aliases)
        
        logger.info(f"Successfully extracted {len(aliases)} aliases including {len(additional_aliases)} generated variations")
        # Pass False to keep organization names in the aliases (default behavior now)
        return filter_aliases(aliases, organizations, remove_org_names=False)
    except Exception as e:
        logger.error(f"Error getting aliases: {str(e)}")
        # Return a minimal set of aliases on error
        return [dataset_name]

def generate_more_alias_variations(aliases: List[str], dataset_name: str) -> List[str]:
    """
    Generate additional variations of aliases based on common patterns.
    This function should be dataset-agnostic and not include hardcoded patterns
    for specific datasets.
    
    Args:
        aliases: Existing list of aliases
        dataset_name: Original dataset name
        
    Returns:
        List[str]: Additional alias variations
    """
    # For now, we won't add any hardcoded variations - all variations
    # should come directly from the LLM output
    return []
...
def research_dataset(dataset_name: str, dataset_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Filter aliases to remove redundancy (but don't remove organization names by default)
        aliases = filter_aliases(aliases, organizations, remove_org_names=False)
